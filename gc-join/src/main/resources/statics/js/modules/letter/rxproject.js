...
function changecolor(){
    $(".changecolor tr:visible").each(function (i) {
...
$(document).on("click","#rxprojectMain .j_uiTab9 li",function(){
    ...
    //等级
    var gradeNew = $(".tractive").attr("gradenew");
    //职位编号
    var postId = $(".tractive").attr("postid");
    // 认知 观念
    var bankType = $(".uiTab9-active").attr("bank-type");
    $("#j-tc-center-content").empty();
    $("#j-tc-center-content").load("two/"+thispage,{cardNo:cardNo,gradeNew:gradeNew,postId:postId,bankType:bankType},function(){
        countMiddle();
        countRight();
    });
    $(".layerRtb").animate({
...
$(document).on("click",".anItemBorThree[data-page]",function () {
    ...
    var parameter;
    thirdCommonAlert.load("three/"+thisPage,{cardNo:cardNo,bankId:bankId}, function () {
    	setTimeout(function(){
    		countThree();
    	})
    });
...
$(document).on("click",".anItemBorFour[data-page]",function () {
    ...
    fourCommonAlert.load("four",{type:thisPage}, function () {
    	setTimeout(function(){
    		countThree();
    	})
    });
