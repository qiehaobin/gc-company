...
function addClass(type){
    id = type;
    $(".title_cur").removeClass("title_cur");
    switch (type){
        case "1":
            $(".dailyLi-fr[data-turnpage='1']").addClass("title_cur");
            break;
        case "2":
            $(".dailyLi-fr[data-turnpage='2']").addClass("title_cur");
            break;
    }
}
...
$(document).on("click","#rxprojectMain .j_uiTab9 li",function(){
    /*var ext2 = $(".title_cur").attr("data-turnpage");*/
    var _othisIndex=$(this).index();
    /*id=$(".tractive").attr("id");*/
    $(this).addClass("uiTab9-active").siblings().removeClass("uiTab9-active");
    $("#j-tc-center-content").empty();
    $("#j-tc-center-courseInfo").load("../train/queryCourseInfo",{id:id,ext1:_othisIndex},function(){});
    $("#course_visit").load("course/visitList",{curriculumId:id},function () {});
    countMiddle();
    countRight();
    $(".layerRtb").animate({
        left: '-100%',
...
function clickTr(obj){
    ...
    id=$(".tractive").attr("id");
    //二段 信息栏  的展示
    $(".j_uiTab9 li").eq(2).click();
    /*$("#j-tc-center-courseInfo").load("../train/queryCourseInfo",{id:id},function(){});*/

}

//添加课程
$(document).on("click","#addTrainCurriculumCommon",function () {
    var options=$("#course_roleId option:selected");
    var ext1 =options.text(); //获取选中的项  //角色名称
    var roleId = $("#course_roleId").val();
    var curriculumName = $("#curriculumName").val();
    var property = $("#property").val();
    var method = $("#method").val();
    var shape = $("#shape").val();
    var speechcraft = $("#speechcraft").val();
    var levelFlag = $("#train_level_flag").val();
    var roughFlag = $(".title_cur").attr("data-turnpage");
    if(curriculumName == null || curriculumName == ""){
        alert("请添加课程名称！");
        return;
    }else{//课程添加
        $.post("../train/addTrainCurriculumCommon",
            {roughFlag:roughFlag,roleId:roleId,curriculumName:curriculumName,property:property,method:method,shape:shape,speechcraft:speechcraft,levelFlag:levelFlag,ext1:ext1},function(data){
            if(data != 0){
                $('.tc-center-show').show();//
