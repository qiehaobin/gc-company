...
$(document).on("click","#rxprojectMain .j_uiTab9 li",function(){
    ...
    $("#j-tc-center-content").load("two/"+thispage,{gradeId:gradeId,bankType:bankType},function(){
        countMiddle();
        countRight();
    });
    $(".layerRtb").animate({
...
$(document).on("click",".anItemBorThree[data-page]",function () {
    ...
    //等级
    var ext1 = $(".tractive").attr("data-level-id");
    var thisPage=$(this).attr("data-page");
    //关闭当前的三段页面
    ...
    thirdCommonAlert.show().removeClass("hide").stop(true, true).animate({
        left: "0"
    }, 200);
    var parameter;
    thirdCommonAlert.load("three/"+thisPage,{id:id,ext1:ext1}, function () {
    	setTimeout(function(){
    		countThree();
    	})
    });
...
$(document).on("click",".anItemBorFour[data-page]",function () {
    ...
    //等级id
    var levelId = $(".tractive").attr("data-level-id");
    //var id = $(this).attr("question-id");
    var thisPage = $(this).attr("data-page");
    $(this).addClass("anItemFour-active").siblings().removeClass("anItemFour-active");
    //三段页面加载位置
    var fourCommonAlert = $(".layerRtb[data-type='fourCommonAlert']");
    fourCommonAlert.show().removeClass("hide").stop(true, true).animate({
        left: "0"
    }, 200);
    fourCommonAlert.load("four/"+thisPage,{id:id,levelId:levelId}, function () {
        setTimeout(function(){
            countThree();
        })
    });
