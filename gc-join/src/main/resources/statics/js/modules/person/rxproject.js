...
$(document).on("click","#rxprojectMain .j_uiTab9 li",function(){
    ...
    var index = $(".uiTab9-active").attr("index");
    $("#j-tc-center-content").empty();
    if(index == 2){
        url = "https://gnr.rxjy.com/hrms/internalPersonal/getUserMsg?curUserCard=00000079&Userno="+Userno+"&AreaId="+AreaId;
        $("#j-tc-center-content").load(url,function(){
            countMiddle();
            countRight();
        });
    }else{
        $("#j-tc-center-content").load("two/"+thispage,{cardNo:$(".tractive").attr("id")},function(){
            countMiddle();
            countRight();
        });
    }
...
$(document).on("click",".anItemBorThree[data-page]",function () {
    ...
    // 岗位 id
    var postId = $(".tractive").attr("post-id");
    // 级别
    var gradeId = $(".tractive").attr("grade-new");
    var thisPage=$(this).attr("data-page");
    // 题库类型  1  认知类型  2观念  3职责  4 专业
    var bankType = $(".anItemBor-active").attr("bank-type");
    // 卡号
    var cardNo = $(".tractive").attr("id");
    thirdCommonAlert.load("three/"+thisPage,{postId:postId,gradeId:gradeId,bankType:bankType,cardNo:cardNo}, function () {
    	setTimeout(function(){
    		countThree();
    	})
    });
